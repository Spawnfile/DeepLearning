...
class ResNet(nn.Module): # [3,4,6,3] block repeating numbers based on paper
    def __init__(self, block, layers, image_channels, num_classes):
        ...
        # ResNet layers (inside of blocks)
        self.layer1 = self._make_layer(block, layers[0], out_channels=64, stride=1)
        self.layer2 = self._make_layer(block, layers[1], out_channels=128, stride=2)
        self.layer3 = self._make_layer(block, layers[2], out_channels=256, stride=2)
        self.layer4 = self._make_layer(block, layers[3], out_channels=512, stride=2)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1)) # we define a output shape(1x1) for tensor
...
def test():
    model = ResNet101(image_channels=3, num_classes=1000)
